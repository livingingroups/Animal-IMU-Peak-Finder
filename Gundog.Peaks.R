...
Gundog.Peaks = function(TS, x, thresh = 0, LoM = 5, constant = "med", ME = 1, plot = TRUE, outlier = FALSE, w = 100, peaks = "positive") {  
  ...
  if (length(constant) != 1) {
    # Pre-compute range and quantile values (constant for all iterations)
    max_value <- range(x[ME > 0], finite = TRUE)[2]
    
    if (outlier != FALSE) {
      quantile_value <- quantile(x[ME > 0], outlier, na.rm = TRUE)
    }
    
    # Vectorized calculation of ref and SHT
    ...
    if (outlier != FALSE) {
      ref <- quantile(x[ME > 0], outlier, na.rm = TRUE) - constant
    }
    SHT <- ref * (abs(thresh) / 100)
  }
